...
public class CollectionWindow : IPanel
{
    ...
    private const int CARD_WIDTH = 130;
    private const int CARD_HEIGHT = 50;
    ...
    private void DrawCard(SpriteBatch spriteBatch, Rectangle rect, GachaColor gachaColor, ColorRarity rarity)
    {
        ...
        spriteBatch.DrawRectangle(rect, owned ? borderColor : Color.Gray,
            owned && rarity >= ColorRarity.Epic ? 3f : 1.5f);
        ...
        int textMaxWidth = rect.Right - (swatchRect.Right + 8) - 4;
        ...
        if (owned)
        {
            ...
            string nameText = TruncateToFit(gachaColor.Name, textMaxWidth);
            spriteBatch.DrawString(font, nameText, new Vector2(swatchRect.Right + 8, rect.Y + 8), Color.Black);
        ...
        else
        {
            ...
            string questionMark = "?";
            Vector2 qSize = font.MeasureString(questionMark);
            spriteBatch.DrawString(font, questionMark,
                new Vector2(swatchRect.X + (swatchRect.Width - qSize.X) / 2f,
                    swatchRect.Y + (swatchRect.Height - qSize.Y) / 2f),
                Color.White);
