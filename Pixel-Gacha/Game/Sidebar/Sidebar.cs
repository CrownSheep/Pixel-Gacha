...
public class Sidebar : IUpdatable, IDrawable
{
    ...
    public Sidebar(GraphicsDevice gd, BitmapFont font, PlayerInventory inventory, DrawingCanvas canvas, ArtworkGallery gallery)
    {
        this.font = font;
        ...
        panels = new Dictionary<SidebarTab, IPanel>
        {
            ...
            { SidebarTab.Gallery,    new GalleryWindow(gd, gallery, canvas, font) },
        };
    }
    ...
    public void SetBounds(Rectangle newBounds)
    {
        ...
        var contentRect = new Rectangle(bounds.X, bounds.Y + TAB_BAR_HEIGHT + headerHeight, bounds.Width, bounds.Height - TAB_BAR_HEIGHT - headerHeight);
    ...
    public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
    {
        ...
        foreach (var (tab, rect) in tabButtons)
        {
            ...
            spriteBatch.FillRectangle(rect, ButtonUtil.GetColor(rect, active ? Color.White : new Color(210, 210, 220)));
            spriteBatch.DrawRectangle(rect, Color.Black);
            spriteBatch.DrawString(font, tab.ToString(), new Vector2(rect.Center.X, rect.Center.Y) - font.MeasureString(tabName) / 2, Color.Black);
        }

        // spriteBatch.DrawRectangle(bounds.X, separatorY, bounds.X + bounds.Width, separatorY, Color.Gray, 1);
        spriteBatch.DrawString(font, currencyString, new Vector2(bounds.X + 8, bounds.Y + TAB_BAR_HEIGHT + HEADER_PADDING_TOP), Color.Green);
